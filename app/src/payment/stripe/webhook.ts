...
async function saveSuccessfulOneTimePayment(
  ...
  const { numOfCreditsPurchased, numOfVideoCreditsPurchased } = getPlanEffectPaymentDetails({ planId, planEffect: plan.effect });
...
function getPlanEffectPaymentDetails({
  ...
  switch (planEffect.kind) {
    case 'subscription':
      return { subscriptionPlan: planId, numOfCreditsPurchased: undefined, numOfVideoCreditsPurchased: undefined };
    case 'credits':
