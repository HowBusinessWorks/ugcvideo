...
async function handleCheckoutSessionCompleted(
  ...
  const isSuccessfulSubscription = session.mode === 'subscription' && session.payment_status === 'paid';

  if (isSuccessfulOneTimePayment) {
    await saveSuccessfulOneTimePayment(session, prismaUserDelegate);
  } else if (isSuccessfulSubscription) {
    await saveSuccessfulSubscription(session, prismaUserDelegate);
  }
}
...
async function saveSuccessfulOneTimePayment(
  ...
  if (session.metadata && session.metadata.type === 'dynamic_credit_purchase') {
      ...
      return updateUserStripePaymentDetails(
        {
          ...
          numOfVideoCreditsPurchased: creditQuantity,
          datePaid: new Date(),
        },
        prismaUserDelegate
      );
  ...
  const { numOfCreditsPurchased, numOfVideoCreditsPurchased } = getPlanEffectPaymentDetails({
    planId,
    planEffect: plan.effect,
  });
  return updateUserStripePaymentDetails(
    { userStripeId, numOfCreditsPurchased, numOfVideoCreditsPurchased, datePaid: new Date() },
    prismaUserDelegate
  );
}

async function saveSuccessfulSubscription(
  session: SessionCompletedData,
  prismaUserDelegate: PrismaClient['user']
) {
  const userStripeId = session.customer;
  ...
  // Set subscription status and plan, but DON'T grant credits here
  // Credits will be granted by invoice.paid event (which fires for both initial purchase and renewals)
  return updateUserStripePaymentDetails(
    {
      userStripeId,
      datePaid,
      ...
      // numOfVideoCreditsPurchased is NOT set here to avoid double-crediting
      // invoice.paid will handle granting credits for both initial purchase and monthly renewals
    },
    prismaUserDelegate
  );
}
...
async function saveActiveSubscription(invoice: InvoicePaidData, prismaUserDelegate: PrismaClient['user']) {
  ...
  // Get the plan and check if it includes monthly credits
  const plan = paymentPlans[subscriptionPlan];
  const { numOfVideoCreditsPurchased } = getPlanEffectPaymentDetails({
  ...
  return updateUserStripePaymentDetails(
    {
      ...
      subscriptionPlan,
      subscriptionStatus: SubscriptionStatus.Active,
      numOfVideoCreditsPurchased, // Grant monthly credits
    },
    prismaUserDelegate
  );
}
    ...
    if (subscription.cancel_at_period_end) {
      if (user.email) {
        try {
          ...
          // In development, SendGrid may not be configured - just log and continue
          console.log('Could not send cancellation email (this is expected in development):', error instanceof Error ? error.message : 'Unknown error');
        }
      }
    }
    return user;
...
function getPlanEffectPaymentDetails({
  ...
  numOfVideoCreditsPurchased: number | undefined;
} {
  switch (planEffect.kind) {
    case 'subscription':
      // Subscriptions include monthly video credits
      return {
        subscriptionPlan: planId,
        numOfCreditsPurchased: undefined,
        numOfVideoCreditsPurchased: planEffect.monthlyCredits, // Grant monthly credits
      };
    case 'credits':
      return { subscriptionPlan: undefined, numOfCreditsPurchased: planEffect.amount, numOfVideoCreditsPurchased: undefined };
    case 'videoCredits':
      return { subscriptionPlan: undefined, numOfCreditsPurchased: undefined, numOfVideoCreditsPurchased: planEffect.amount };
    default:
