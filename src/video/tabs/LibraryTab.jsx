...
const AssetSkeleton = () => (<Card className="overflow-hidden">
...
export default function LibraryTab() {
    ...
    const handleRefundRequest = async (generationId) => {
    ...
    return (<div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between border-b pb-3">
        <div className="flex gap-2">
          {filters.map((filter) => {
            ...
            return (<Button key={filter.id} variant={isActive ? 'default' : 'outline'} size="sm" onClick={() => handleFilterChange(filter.id)} className="gap-2">
                {Icon && <Icon className="w-4 h-4"/>}
                {filter.label}
              </Button>);
        })}
        </div>
        {totalCount > 0 && (<p className="text-sm text-muted-foreground">
      ...
      {isLoading && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(6)].map((_, i) => (<AssetSkeleton key={i}/>))}
        </div>)}
      ...
      {/* Empty State */}
      {!isLoading && !error && assets?.length === 0 && (<div className="text-center py-12">
      ...
      {!isLoading && !error && assets && assets.length > 0 && (<>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {assets.map((asset) => {
                ...
                if (asset.status === 'FAILED') {
                    return (<Card key={asset.id} className="overflow-hidden border-red-200 bg-red-50/30">
                    ...
                    <CardContent className="p-4 space-y-3">
                      ...
                      {/* Error Message */}
                      {asset.errorMessage && (<div className="bg-red-50 border border-red-200 rounded p-2">
                          <p className="text-xs text-red-700">{asset.errorMessage}</p>
                        </div>)}

                      {/* Action Buttons */}
                      <div className="space-y-2">
                        {asset.isRefundable && !asset.creditsRefunded && (<Button onClick={() => handleRefundRequest(asset.id)} variant="outline" size="sm" className="w-full border-green-300 text-green-700 hover:bg-green-50" disabled={processingRefund === asset.id}>
                        ...
                        {asset.creditsRefunded && (<div className="text-center">
                            <Badge variant="secondary" className="bg-green-100 text-green-700">
                              Credits Refunded
                            </Badge>
                          </div>)}
                        {asset.canRetry && (<Button onClick={() => handleRetry(asset.id)} variant="outline" size="sm" className="w-full" disabled={retrying === asset.id}>
                            {retrying === asset.id ? (<>
                                <Loader2 className="w-3 h-3 mr-1 animate-spin"/>
                                Retrying...
                              </>) : (<>
                ...
                return (<Card key={asset.id} className="overflow-hidden">
                  <div className="aspect-[3/4] bg-gray-100 relative">
                    {url && !isVideo && (<img src={url} alt={getAssetTypeLabel(asset.assetType)} className="w-full h-full object-cover" loading="lazy" decoding="async"/>)}
                    {url && isVideo && (<video src={url} className="w-full h-full object-cover" controls preload="metadata"/>)}
                    {!url && (<div className="w-full h-full flex items-center justify-center">
                  ...
                  <CardContent className="p-4 space-y-3">
                    ...
                    <div className="text-xs text-muted-foreground space-y-1">
                      {asset.assetType === 'PERSON' && asset.stage1Mode && (<p>Mode: {asset.stage1Mode}</p>)}
                      {asset.assetType === 'VIDEO' && asset.veo3Mode && (<p>Quality: {asset.veo3Mode}</p>)}
                    </div>

                    <Button onClick={() => handleDownload(asset)} variant="outline" size="sm" className="w-full">
          ...
          {totalPages > 1 && (<div className="flex items-center justify-between mt-6 pt-6 border-t">
              ...
              <Button variant="outline" size="sm" onClick={() => setCurrentPage(currentPage + 1)} disabled={!hasMore} className="gap-2">
                Next
